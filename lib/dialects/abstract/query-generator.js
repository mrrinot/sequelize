...
class QueryGenerator {
  ...
  */
  selectQuery(tableName, options, model) {
    ...
    if (options.group) {
      console.log('KDQNJK3,', options.aliasesByTable, mainTable.as, options.group);
      options.group = Array.isArray(options.group) ? options.group.map(t => this.aliasGrouping(t, model, mainTable.as, options)).join(', ') :  this.aliasGrouping(options.group, model, mainTable.as, options);
      console.log('DKNK3?', options.group);
      if (subQuery) {
  ...
  aliasGrouping(field, model, tableName, options) {
    ...
    if (this.options.minifyAliases && options.aliasesByTable[tableName] && options.aliasesByTable[tableName][field])
    {
      alias = options.aliasesByTable[tableName][field];
    }

    return this.quote(alias, model);
  }
  ...
  escapeAttributes(attributes, options, mainTableAs) {
    return attributes && attributes.map(attr => {
      ...
      if (Array.isArray(attr)) {
        ...
        if (this.options.minifyAliases) {
          console.log('ALIAS', alias, mainTableAs);
  ...
  generateInclude(include, parentTableName, topLevelInfo) {
    ...
    if (topLevelInfo.options.includeIgnoreAttributes !== false) {
      ...
      const includeAttributes = include.attributes.map(attr => {
        ...
        if (this.options.minifyAliases) {
          console.log('ALIAS 2', alias, includeAs.internalAs);
  ...
  _getMinifiedAlias(alias, tableName, options) {
    ...
    if (!options.aliasesByTable[tableName]) {
      options.aliasesByTable[tableName] = {};
    }
  ...
  generateJoin(include, topLevelInfo) {
    ...
    console.log('KDZQ', joinOn);
    if (topLevelInfo.options.groupedLimit && parentIsTop || topLevelInfo.subQuery && include.parent.subQuery && !include.subQuery) {
      console.log('PARENT IS TOP ???', parentIsTop);
      if (parentIsTop) {
        ...
        console.log('PARENT IS TOP', topLevelInfo.options.aliasesByTable, tableName, alias);
        if (this.options.minifyAliases && topLevelInfo.options.aliasesByTable[tableName] && topLevelInfo.options.aliasesByTable[tableName][alias])
        {
          alias  = topLevelInfo.options.aliasesByTable[tableName][alias];
        }

        joinOn = `${tableName}.${this.quoteIdentifier(alias)}`;
      } else {
        const joinSource = `${asLeft.replace(/->/g, '.')}.${attrLeft}`;

        console.log('PARENT NOT TOP', topLevelInfo.options.aliasesByTable, asLeft, joinSource);
        if (this.options.minifyAliases && topLevelInfo.options.aliasesByTable[asLeft] && topLevelInfo.options.aliasesByTable[asLeft][joinSource])
        {
          joinOn = topLevelInfo.options.aliasesByTable[asLeft][joinSource];
        }
        else 
        {
  ...
  generateThroughJoin(include, includeAs, parentTableName, topLevelInfo) {
    ...
    const throughAttributes = through.attributes.map(attr => {
      ...
      if (this.options.minifyAliases) {
        console.log('ALIAS 3', alias, throughAs);
    ...
    console.log('NFQEI', attrSource, tableSource, topLevelInfo.options.aliasesByTable, `${tableSource}.${attrSource}`);
    ...
    if (topLevelInfo.subQuery && !include.subQuery && include.parent.subQuery && !parentIsTop) {
      let joinSource = `${tableSource}.${attrSource}`;
      ...
      if (this.options.minifyAliases && topLevelInfo.options.aliasesByTable[tableSource] && topLevelInfo.options.aliasesByTable[tableSource][joinSource]) {
        joinSource = topLevelInfo.options.aliasesByTable[tableSource][joinSource];
      }
      ...
      // If we are minifying aliases and our JOIN target has been minified, we need to use the alias instead of the original column name
      if (this.options.minifyAliases && topLevelInfo.options.aliasesByTable[tableSource] && topLevelInfo.options.aliasesByTable[tableSource][attrSource]) {
        attrSource = topLevelInfo.options.aliasesByTable[tableSource][attrSource];
      }
  ...
  getQueryOrders(options, model, subQuery) {
    ...
    if (Array.isArray(options.order)) {
      for (let order of options.order) {
        ...
        console.log('éKDQZNK', this.quote(order, model, '->'));
        if (
          subQuery
          && Array.isArray(order)
          && order[0]
        ...
        console.log('22222222222222', this.quote(order, model, '->'));
