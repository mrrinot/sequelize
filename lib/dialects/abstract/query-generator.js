...
class QueryGenerator {
  ...
  */
  ...
  escapeAttributes(attributes, options, mainTableAs) {
    return attributes && attributes.map(attr => {
      ...
      if (Array.isArray(attr)) {
        ...
        if (this.options.minifyAliases) {
          console.log('ODJZ', alias, mainTableAs, attr[0]);
          alias = this._getMinifiedAlias(alias, `${mainTableAs}.${this.quoteIdentifier(alias)}`, options);
        }
  ...
  generateInclude(include, parentTableName, topLevelInfo) {
    ...
    if (topLevelInfo.options.includeIgnoreAttributes !== false) {
      ...
      const includeAttributes = include.attributes.map(attr => {
        ...
        if (this.options.minifyAliases) {
          console.log('dPQJOZDD', alias, includeAs.externalAs, attr, ' ====> ', `${this.quoteIdentifier(includeAs.externalAs)}.${this.quoteIdentifier(attrAs)}`);
          alias = this._getMinifiedAlias(alias, `${this.quoteIdentifier(includeAs.externalAs)}.${this.quoteIdentifier(attrAs)}`, topLevelInfo.options);
        }
  ...
  _getMinifiedAlias(alias, fullAlias, options) {
    const minifiedAlias = `_${options.aliasesMapping.size}`;

    options.aliasesMapping.set(minifiedAlias, { alias, fullAlias });
  ...
  generateThroughJoin(include, includeAs, parentTableName, topLevelInfo) {
    ...
    const throughAttributes = through.attributes.map(attr => {
      ...
      if (this.options.minifyAliases) {
        console.log('KDNQZK', alias, throughAs, Array.isArray(attr) ? attr[0] : attr);
        alias = this._getMinifiedAlias(alias, `${this.quoteIdentifier(throughAs)}.${this.quoteIdentifier(alias)}`, topLevelInfo.options);
      }
